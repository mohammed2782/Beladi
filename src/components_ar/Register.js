...
export class Register_ar extends  Component <{}> {
	...
	async componentDidMount (){
		...
		let kickout = false;
		const filters = await api.fetchEcommerceFiltersList('AR');
		console.log(filters);
		...
		colorFilter.push({"text":"إلغاء","code":"CNCL"});
		this.setState({colorFilter:colorFilter});
		
		
		if (this.state.custId !=null && this.state.custId !==undefined){
			const customerInfo = await api.getRegisterCustomerInfoBeladi(this.state.custId,'AR');
	...
	async _confirmedCancel(){
		...
		try{
			...
			await api.cancelRegisterationBeladi(zuidInPhone, this.state.custId, "AR");
	...
	async _registerCustomer(){
		...
		if (custVin !=='' && custVin !== undefined && custVin !==null && custVin !== 'undefined'){
			if (custVin.length ==17){
				...
				Alert.alert('خطأ',"رقم الشاصي يتكون من 17 حرف");
				allGood = false;
				vinEmptyF = true;
			}
		}else{
		...
		if (allGood){
			...
			this.setState({isLoading:true});
			try{
				const result = await api.registerCustomerBeladi(zuid,custId,brand, model, color, year, custName, custHP1, custEmail, custVin , this.state.playerid, 'AR'  );
				...
				if (result.errorMessage!==null && result.errorMessage!==undefined)
					...
					if (result.custId !=null && result.custId !==undefined){
						...
						const customerInfo = await api.getRegisterCustomerInfoBeladi(result.custId,'AR');
	...
	_lookupModelFiler (choice){
		if (choice == 'CNCL'){
			...
			if (modelFilterF.length>0)
				...
				this.setState({modelDisabled:true});
			this.setState({modelFilter:modelFilterF,model:"الموديل", modelChoice:null});
							...
							<View >
								<View style={{flex:6 , marginLeft:30, marginRight:30,backgroundColor:"white",
									...
									<Content padder>
										...
										<View style={{flexDirection:"row-reverse",flex:1,marginTop:10,alignItems:"center",justifyContent:"center",}}>
													...
													onPress={() =>	
															...
															buttonIndex => {
																if (this.state.yearFilter[buttonIndex].code =='CNCL'){
																	this.setState({year:"السنه", yearChoice:null});
																}else{
													...
													onPress={() =>	
															...
															buttonIndex => {
																if (this.state.colorFilter[buttonIndex].code =='CNCL'){
																	this.setState({color:"اللون", colorChoice:null});
																}else{
										...
										<View style={{flexDirection : "row" , justifyContent:"center" , alignItems:"center", flex:1}}>
											<Button block onPress={()=> {this._registerCustomer();}}
												...
												<Text style={{color:"white"}}>سجل</Text>
											</Button>
										</View>
				...
				<ImageBackground style={styles.backgroundImage}
						...
						<View style={{flex:3 , marginLeft:30, marginRight:30,marginTop:"5%",paddingTop:10}}>
							...
							<View style={{flexDirection:"row-reverse"}} >
								...
								<View style={styles.formRow,{flexDirection:"row-reverse"}}>
									<Label style={styles.labelStyle}>
										رقم الشاصي:  
									</Label>
									<Text>
								...
								<View style={{flexDirection : "row" , justifyContent:"center" , alignItems:"center", flex:1}}>
									<Button iconLeft onPress={()=> {this._updateRegInfo();}}
										...
										<View style={{flex:1,alignItems:"center",justifyContent:"center" }}>
											...
											<Text style={{color:"white",fontSize:15,marginRight:5,fontWeight:"bold"}}>تعديل</Text>
										</View>
									</Button>
									<Button iconLeft onPress={()=> {this._cancelReg();}}
										style={{backgroundColor:"#C72E2E", flex:1, margin:0, marginTop: 10, padding:5,}}>
										<View style={{flex:1,alignItems:"center",justifyContent:"center" }}>
											<Text style={{color:"white",fontSize:15,marginRight:5,fontWeight:"bold",alignSelf:"center",alignItems:"center",justifyContent:"center"}}>إلغاء</Text>
											<Text style={{color:"white",fontSize:15,marginRight:5,fontWeight:"bold",alignSelf:"center",alignItems:"center",justifyContent:"center"}}>التسجيل</Text>
										</View>
									</Button>
								</View>
...
const styles = StyleSheet.create(
	{
		...
		textInputStyle :{  borderColor: 'gray',textAlign:"right" , backgroundColor :"#FFF",  flex:1 , fontSize :12 , paddingTop:10 ,},
		...
		actionBrandYear : {flex:1,backgroundColor:"#004777",height:40,alignSelf:"center",borderRadius:5,paddingTop:0,paddingBottom:0},
