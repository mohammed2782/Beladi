...
//import SearchableDropdown from 'react-native-searchable-dropdown';
	...
	export  class EcommerceMaster_ar extends Component <{}> {
		constructor (props){
			super(props);
			//alert("this is const");
			this.state ={
...
//alert(itemid);
		...
		async _doSearch(){
			...
			try{
				...
				const itemsList = await api.fetchEcommereceList('AR', brand, model , system, year);
		...
		async componentDidMount(){
			//alert("this is json1 ");
			const itemsList = await api.fetchEcommereceList('AR','','','','');
							...
							<View style={{flexDirection:"row-reverse",flex:1.5, alignItems:"center"}}>
								...
								<View  style={{flex:1,paddingLeft:30}}>
									<Button transparent onPress={this.state.itemsInCart?
											() => this.props.navigation.navigate("EcommerceCart_ar",
