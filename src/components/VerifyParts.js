...
titleYpos = new Animated.Value(0);
    ...
    onBarCodeRead = (e) =>{ 
		Vibration.vibrate(200);this.setState({qrcode: e.data,borderColor:"green",found:true}); 
	...
	async _checkPart(){
		...
		if (allGood){
			...
			this.setState({isLoading:true});
			try{
				const result = await api.checkPart(qrcode,"EN");
			...
			if (this.state.scanLimit!=null && this.state.scanLimit){
				return (<View style={{flex:2,flexDirection:"row",justifyContent:"center",marginTop:20}}>
							...
							<View style={{flex:5,alignItems:"flex-start",}}>
								<Text style={[styles.verified,{paddingTop:5,fontSize : 13,color:"#CD7300"}]}>{this.state.serverMsg}</Text>
							</View>
						</View>);
		...
		if (this.state.verified !=null){
			if (this.state.scanLimit!=null && this.state.scanLimit){
				return  (<View style={{flex:1,flexDirection:"column",justifyContent:"center",marginTop:20}}>
						...
						<Text style={{color: 'white',fontSize:15}}>Open camera</Text>
					</Button>
						</View>);
					...
					<View style={{flex:4,backgroundColor:"#004777"}}>
						...
						<View style={{flex:2,flexDirection:"row",alignItems:"center"}}>
							<View style={{flex:7,alignItems:"flex-end",}}>
								<Text style={{fontSize:14,color:"white"}}>Scan your barcode or QR code here</Text>
							</View>
							<View style={{flex:0.3,}}>
							</View>
									...
									<View style={{flex:1,justifyContent:"space-between",alignItems:"center",marginTop:20}}>
										...
										<View style={{flex:2}}>
											<Button block onPress={()=> {this._checkPart();}}
												...
												<Text style={{color:"white",fontSize:20}}>Check Now!</Text>
											</Button>
										</View>
									</View>
								</TouchableWithoutFeedback>
