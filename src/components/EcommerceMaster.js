...
//import SearchableDropdown from 'react-native-searchable-dropdown';
	...
	export  class EcommerceMaster extends Component <{}> {
		constructor (props){
			super(props);
			//alert("this is const");
			this.state ={
...
//alert(itemid);
		...
		async _doSearch(){
			...
			try{
				...
				const itemsList = await api.fetchEcommereceList('en', brand, model , system, year);
		...
		async componentDidMount(){
			//alert("this is json1 ");
			const itemsList = await api.fetchEcommereceList('en','','','','');
			...
			systemFilter.push({"text":"Cancel","code":"CNCL"});
			...
			<Root>
				<ImageBackground style={styles.backgroundImage}
					...
					<View style={{flexDirection:"column" , flex:1 , justifyContent:"flex-start"}}>
						<View style={{flex:3, backgroundColor :"#004777", paddingLeft:0,borderBottomRightRadius:30}}>
							...
							<View style={{flexDirection:"row",flex:1.5, alignItems:"center"}}>
								<View style={{flex:2}}>
									<Text style={{color:"white", fontSize:12, paddingLeft:20}}>Find Parts</Text>
								</View>
								
								<View  style={{flex:1}}>
									<Button transparent onPress={this.state.itemsInCart?
											() => this.props.navigation.navigate("EcommerceCart",
										...
										onPress={() =>	ActionSheet.show({
													...
													buttonIndex => {
														if (this.state.systemFilter[buttonIndex].code =='CNCL'){
															this.setState({system:"Category", systemChoice:null});
														}else{
																	...
																	<View style={{flex:3}}>
																		...
																		<View style={{flex:1,flexDirection:"row",marginTop:10,marginBottom:0,justifyContent:"center"}}>
																			...
																			alignItems:"center",justifyContent:"center", borderRadius:5,}}>
																				<Text style={{color:"white",fontSize:11,marginLeft:0,paddingLeft:0,paddingRight:0}}>Add to cart</Text>
																			...
																			<View style={{flex:12,paddingLeft:10,marginBottom:0}}> 
																				<Text style={styles.oldPriceText}>
																					 {util.formatNumber(item.itemOldPrice)} Old Price
																				</Text>
																				<Text style={styles.newPriceText}>
																					 {util.formatNumber(item.itemPrice)} Now
																				</Text>
