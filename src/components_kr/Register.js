...
export class Register_kr extends  Component <{}> {
	...
	async componentDidMount (){
		...
		if (this.state.custId !=null && this.state.custId !==undefined){
			const customerInfo = await api.getRegisterCustomerInfoBeladi(this.state.custId,'KR');
	...
	async _confirmedCancel(){
		...
		try{
			...
			await api.cancelRegisterationBeladi(zuidInPhone, this.state.custId, "KR");
	...
	async _registerCustomer(){
		...
		if (allGood){
			...
			this.setState({isLoading:true});
			try{
				const result = await api.registerCustomerBeladi(zuid,custId,brand, model, color, year, custName, custHP1, custEmail, custVin , this.state.playerid, 'KR'  );
				
				if (result.errorMessage!==null && result.errorMessage!==undefined)
					Alert.alert('تۆمارکردن',result.errorMessage);
				else{
					//store in AsyncStorage
					if (result.custId !=null && result.custId !==undefined){
						this.setState({updateInfo:false});
						//console.log("load info");
						const customerInfo = await api.getRegisterCustomerInfoBeladi(result.custId,'KR');
									...
									<Content padder>
													...
													onPress={() =>	
															...
															buttonIndex => {
																if (this.state.modelFilter[buttonIndex].code =='CNCL'){
																	this.setState({model:"مۆدەل", modelChoice:null});
																}else{
													...
													onPress={() =>	
															...
															buttonIndex => {
																if (this.state.colorFilter[buttonIndex].code =='CNCL'){
																	this.setState({color:"ڕەنگ", colorChoice:null});
																}else{
										...
										<View style={{flexDirection : "row" , justifyContent:"center" , alignItems:"center", flex:1}}>
											<Button block onPress={()=> {this._registerCustomer();}}
												...
												<Text style={{color:"white"}}>تۆمارکردن</Text>
											</Button>
										</View>
								...
								<View style={{flexDirection : "row" , justifyContent:"center" , alignItems:"center", flex:1}}>
									<Button iconLeft onPress={()=> {this._updateRegInfo();}}
										...
										<View style={{flex:1,alignItems:"center",justifyContent:"center" }}>
											...
											<Text style={{color:"white",fontSize:15,marginRight:5,fontWeight:"bold"}}>دەگۆڕێت</Text>
										</View>
									</Button>
									<Button iconLeft onPress={()=> {this._cancelReg();}}
										style={{backgroundColor:"#C72E2E", flex:1, margin:0, marginTop: 10, padding:5,}}>
										<View style={{flex:1,alignItems:"center",justifyContent:"center" }}>
											<Text style={{color:"white",fontSize:15,marginRight:5,fontWeight:"bold",alignSelf:"center",alignItems:"center",justifyContent:"center"}}>سڕینه‌وه‌ی</Text>
											<Text style={{color:"white",fontSize:15,marginRight:5,fontWeight:"bold",alignSelf:"center",alignItems:"center",justifyContent:"center"}}>تۆمار</Text>
										</View>
									</Button>
								</View>
