...
import {
  ...
  Thumbnail, CardItem, Badge, Item, Input
	...
	export  class EcommerceCart_kr extends Component <{}> {
		constructor (props){
			super(props);
			//alert("this is const");
			this.state ={
		...
		async _buyItems(){
			...
			if (allGood){
				...
				try{
					const result = await api.purchaseItems(zuid, custName, custHP, custEmail, custNotes, cart , 'KR');
					if (result.errorMessage!==null && result.errorMessage!==undefined)
						alert(result.errorMessage);
					else{
						await Alert.alert("","تمت عملية الشراء, سوف نتصل بكم قريبا");
				...
				<ImageBackground style={styles.backgroundImage}
					...
					<View style={{flexDirection:"column" , flex:1 , justifyContent:"flex-start"}}>
						...
						<View style={{flex:8, marginTop:5 , marginLeft:35, marginRight:35}}>
							<View style={{flexDirection:"row-reverse",backgroundColor:"#7797ad",borderTopLeftRadius:7,borderTopRightRadius:7,justifyContent:"center", alignItems:"center"}}>
								<View style={{flex:1,justifyContent:"center",alignItems:"center",paddingTop:5,paddingBottom:5}}>
									<Text style={{fontSize:11,fontWeight: 'bold',color:"#b7b7b7"}}>القطعه</Text>
								</View>
								<View style={{flex:1,justifyContent:"center",alignItems:"center"}}>
									<Text style={{fontSize:11,fontWeight: 'bold',color:"#b7b7b7"}}>المجموع</Text>
								</View>
							</View>
							<View>
								<FlatList 
																		...
																		<View style={{flex:3}}>
																			<View style={{flex:10,flexDirection:"row-reverse"}}>
																				<View style={{flex:3,paddingLeft:10}}>
																					...
																					<View style={{flex:4,marginTop:5,flexDirection:"row-reverse",alignItems:"center",}}>
																						<Text note style={{flex:1,fontSize:10,fontWeight: 'bold',color:"#b7b7b7"}}>العدد:{'  '} 
																						<Text note style={{flex:1,fontSize:10,fontWeight: 'bold',color:"#ad1f2c",marginLeft:5}}>{item.count}</Text></Text>
																						<View style={{flex:1,marginRight:20, justifyContent : "flex-end"}}>
																							<Button	block style={{justifyContent : "center",paddingTop:0,height:20, paddingBottom:0,backgroundColor:"#ad1f2c",borderRadius:7}} 
																								onPress={() => this._removeItem(item.id)}>
																								<Text note style={{fontSize:9,fontWeight: 'bold',color:"white",marginLeft:5}}>حذف</Text>
																							</Button>
																						</View>
																					</View>
																				</View>
							...
							<View style={{borderBottomLeftRadius:7,borderBottomRightRadius:7,flexDirection:"row-reverse",
							justifyContent:"center",alignItems:"center",backgroundColor:"white"}}>
								
								<Text style={{color:"black", fontSize:14, fontWeight:"bold"}}>المجموع الكلي{'  '}</Text>
							...
							<View style={{backgroundColor:"white",marginTop:10,borderRadius:7,}}>
									...
									<Form style={styles.form}>
										<Item style={[this.state.nameEmpty?styles.validationError:null,{flexDirection:"row-reverse"}]}>
										...
										<Item style={[this.state.hpEmpty?styles.validationError:null,{flexDirection:"row-reverse"}]}>
										...
										<Item style={[this.state.custEmailError?styles.validationError:null,{flexDirection:"row-reverse"}]}>
							...
							<View style={{marginTop:10,flex:1,borderRadius:7,backgroundColor :"#FFF",}}>
								...
								<Input multiline={true} numberOfLine={5} name = "msg"  placeholder="ملاحظات"
							...
							<View style={{flexDirection : "row" , justifyContent:"center" , alignItems:"center", flex:1}}>
											<Button block onPress={()=> {this._buyItems();}}
												...
												<Text style={{color:"white"}}>شراء</Text>
											</Button>
							</View>
						</View>
					</View>
	...
	const styles = StyleSheet.create(
		{
			...
			formRow : {flexDirection : "row-reverse" , justifyContent:"flex-start" , alignItems:"center", flex:1,backgroundColor:"white" },
			oldPriceText : {
			...
			textInputStyle :{  borderColor: 'gray',backgroundColor :"#FFF", textAlign:"right", flex:1 , fontSize :12 , paddingTop:5 ,},
			newPriceText :{ 
				textDecorationStyle: 'solid',
				fontFamily : "Franklin Gothic Book Regular",
				fontSize : 12,
			...
			multiLineInputStyle : {height:150 ,  flex:1 , fontSize :10 , lineHeight:10 ,textAlign:"right",  marginBottom : 10 , textAlignVertical: "top"},
