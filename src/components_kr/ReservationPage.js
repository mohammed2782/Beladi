...
export class ReservationPage_kr extends  Component <{}> {
	...
	async componentDidMount(){
		try{
			...
			if (!isThere){
				await Alert.alert('','يجب أن تسجل لتقوم بالحجز لموعد صيانه');
				this.props.navigation.navigate("Register_kr",{appCallBackFunction:this.registerUser, unRegCallBackFunction:this.unRegisterUser});
				return ;
			}
			let regidD = 0;
			...
			const reservationDates = await api.fetchOldNewReservationDate('KR',this.state.zuid,this.state.regid);
	...
	async _contactSend(){
		...
		if (allGood){
			...
			const result = await api.makeAppointment(this.state.zuid, custName, custHP, custEmail, brand, dateTime, msg,state, svc,this.state.regid, 'kr'  );
							...
							{ (!this.state.makeReservationMode && this.state.haveReservations)?
								...
								<View style={{flexDirection : "row" , justifyContent:"center" , alignItems:"center", flex:1}}>
											<Button block onPress={()=> {this._showResForm();}}
												...
												<Text style={{color:"white"}}>دێارێكردنى كاتێ سەردانێ كردن</Text>
											</Button>
								</View>
							</View>
							:
							...
							<View style={{flex:6 , marginLeft:20,paddingTop:20, marginRight:20}}>
								<Form style={styles.form}>
									<View style={styles.formRow}>
										<Input 
											name = "customerName" placeholder="ناوى كريار"
									...
									<View style={styles.formRow} >
										...
										<View style={{flex:4}}>
											<Input
												...
												name = "custHP" placeholder="ژماره‌ی ته‌له‌فۆن"
									...
									<View style={{marginLeft:0,marginRight:0,flexDirection : "column" ,alignItems:"flex-end", justifyContent:"flex-end" , alignItems:"flex-end", flex:1 }}>
										<View style={{flex:1, paddingRight : 0 , marginBottom:0,justifyContent:"flex-end",alignItems:"flex-end",}}>
											<Text style={{color:"#004777",fontSize:10}}>هەلبژاردنی کات و بەروار</Text>
										</View>
										<View style={{flex:1, paddingRight :0 ,flexDirection:"row-reverse", marginBottom:10,justifyContent:"flex-end",alignItems:"flex-end",}}>
